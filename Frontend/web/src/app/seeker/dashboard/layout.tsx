...
const sidebarSections = [
  ...
  { label: 'Account', items: [
    { href: '/seeker/dashboard/profile', icon: '👤', name: 'My Profile' },
  ]},
];
...
export default function SeekerDashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!isAuthenticated || !user) {
      router.replace('/signin?redirect=/seeker/dashboard');
      return;
    }
  ...
  const isActive = (href: string) => {
    if (href === '/seeker/dashboard') return pathname === '/seeker/dashboard';
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            {avatar ? (
              <img src={avatar} alt={displayName} style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', border: `2px solid ${C.gold}` }} />
            ) : (
              <div style={{ width: 36, height: 36, borderRadius: '50%', border: `2px solid ${C.gold}`, background: C.goldPale, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, color: C.gold }}>
        ...
        <aside style={{
          ...
          <div style={{ padding: '8px 0' }}>
            <Link href="/practitioners"
              ...
              <span style={{ fontSize: '16px', width: '20px', textAlign: 'center' }}>🔍</span> Browse Practitioners
            </Link>
          </div>
        </aside>
