...
export function CheckoutProgress({ steps, current }: CheckoutProgressProps) {
  return (
    <div style={{ background: '#fff', borderBottom: '1px solid rgba(240,120,20,0.1)', padding: '16px 48px' }}>
        ...
        {steps.map((label, i) => {
          ...
          return (
            <div key={i} style={{ display: 'flex', alignItems: 'center' }}>
              {i > 0 && <div style={{ width: 40, height: 1, background: isDone ? '#F07814' : 'rgba(240,120,20,0.2)' }} />}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{
                  width: 24, height: 24, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontSize: 11, fontWeight: 600,
                  background: isDone ? '#5A8A6A' : isActive ? '#F07814' : 'rgba(240,120,20,0.1)',
