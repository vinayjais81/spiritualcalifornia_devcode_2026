...
- ✅ **Batch 2 (done) — `mission` CMS renderer:** decision was **extend the renderer** (keep editable). Found it was **already supported** — `StaticPageRenderer` styles `.steps-box`/`.pillar`, `staticPageBlocks.ts` defines 8 Tiptap round-trip nodes (admins can edit the cards), and 3 migrations seed the content. Aligned styling to exact v6 spec: steps-box `margin` 24→40px + `h3` 22px; pillar `margin-bottom` 16→24px; pillar-icon bg → `#FEF7F0`.
- ✅ **Batch 2 (done) — guide dashboard cosmetic pass:** shared components (`PageHeader`/`Panel`/`StatCard`/`Btn` in `dashboard-ui.tsx`, sidebar in `layout.tsx`) were a **line-by-line match** to the v6 design already (built from an earlier cut + Phase 0 recolor). Only genuine delta added: sidebar-link **`:hover`** state (gold-pale) via mouse handlers. No structural change — all dashboard features intact.
