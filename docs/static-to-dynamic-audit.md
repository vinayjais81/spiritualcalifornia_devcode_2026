...
- [x] **UX** [onboarding/guide, /guide/dashboard] **Guide deferred onboarding shipped (companion to the seeker change).** Email-verified guides skip the 7-step wizard and land on `/guide/dashboard`, where a 5-section `GuideProfileCompletenessWidget` (categories / profile / location & schedule / credentials / submitted-for-verification) routes each section to the appropriate dashboard sub-page. Wizard remains accessible via `?resume=1` for guides who prefer the linear flow. Public visibility gate (`isPublished AND isVerified`) untouched — widget helps guides *reach* verification, doesn't bypass it. No backfill needed (state derived from existing schema). Full spec: [docs/guide-deferred-onboarding.md](./guide-deferred-onboarding.md). ✅ SHIPPED 2026-04-28
- [x] **Policy** [auth.service, guides.service, onboarding/guide, /register] **SEEKER ↔ GUIDE role mutex enforced.** One email = one marketplace role. New `intent` field on `RegisterDto` makes guide registration skip SEEKER assignment; cross-role guard on `/guides/onboarding/start` rejects existing seekers with 409. Friendly block pages on both `/onboarding/guide` (for SEEKERs) and `/register` (for GUIDEs). ADMIN/SUPER_ADMIN exempt. Migration `20260428100000_enforce_seeker_guide_mutex` drops SEEKER role from any dual-role users on deploy (keeps GUIDE — guides have verified profiles, the truer signal of intent). Full spec: [docs/seeker-guide-role-mutex.md](./seeker-guide-role-mutex.md). ✅ SHIPPED 2026-04-28 — ⚠️ **partially superseded 2026-09-09:** "one email = one marketplace role" now holds in one direction only. A seeker still cannot become a practitioner, but practitioners hold `SEEKER` so they can buy from each other. See [practitioners-as-buyers.md](./practitioners-as-buyers.md).
- [x] **Security** [auth.service, /auth/register, /auth/verify-email, /auth/login, /signin, /verify-email, /register, Step1Profile] **Email verification required before login.** `/auth/register` no longer issues tokens; `/auth/verify-email` mints them after the user proves ownership; `/auth/login` rejects unverified accounts with `EMAIL_NOT_VERIFIED`. New `User.pendingIntent` column persists intent across the verify round-trip. Closes the "register with someone else's email → instant session" attack. Full spec: [docs/verify-email-before-login.md](./verify-email-before-login.md). ✅ SHIPPED 2026-04-28
- [x] **Security** [guides.service.getPublicProfile] **Unverified guide profiles hidden from public view.** Direct `/guides/:slug` lookups now require `isVerified=true AND isPublished=true`; otherwise 404 (same shape as a non-existent slug, doesn't leak). The `/practitioners` listing was already correctly gated. ✅ SHIPPED 2026-04-28
- [x] **Bug** [JwtRefreshStrategy] **`/auth/refresh` always returned 401.** Strategy's `validate()` returned the user with `sub` instead of `id`, so controller's `user.id` was undefined and `refreshTokens(undefined, …)` always threw. Added explicit `sub → id` mapping to match `JwtStrategy`. Latent bug exposed by the recent guide-onboarding flow. ✅ SHIPPED 2026-04-28
...
- **2026-04-28** — Seeker deferred onboarding. Email-verified seekers no longer get trapped in the wizard if they leave mid-flow; `/register` redirects them to the dashboard, where a self-hiding `ProfileCompletenessWidget` nudges them to fill the deferred wizard fields (`experienceLevel`, `practices`, `journeyText`) via the profile page. New columns + idempotent backfill in one migration. Full spec at [seeker-deferred-onboarding.md](./seeker-deferred-onboarding.md). **Same-day companion:** Guide deferred onboarding — same redirect-to-dashboard rule on `/onboarding/guide`, plus a 5-section `GuideProfileCompletenessWidget` on `/guide/dashboard` whose chips deeplink to the relevant dashboard sub-pages (profile / location / verification). Wizard remains accessible via `?resume=1`. No DB changes — state derived from existing schema. Full spec at [guide-deferred-onboarding.md](./guide-deferred-onboarding.md). **Same-day policy fix:** SEEKER ↔ GUIDE role mutex enforced — one email = one marketplace role. Backend `intent` parameter, cross-role guard, friendly UI blocks both directions, idempotent migration to drop SEEKER from any dual-role users. Full spec at [seeker-guide-role-mutex.md](./seeker-guide-role-mutex.md).
