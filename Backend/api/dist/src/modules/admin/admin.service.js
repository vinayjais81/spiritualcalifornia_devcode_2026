...
let AdminService = class AdminService {
    ...
    async approveGuide(guideId) {
        ...
        if (guide.verificationStatus !== client_1.VerificationStatus.PENDING) {
            throw new common_1.BadRequestException('Guide is not in pending status');
        }
    ...
    async rejectGuide(guideId, reason) {
        const guide = await this.prisma.guideProfile.findUnique({
            where: { id: guideId },
        });
        if (!guide)
    ...
    async getFinancials(params) {
        ...
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const [totalRevenueAgg, monthlyRevenueAgg, totalPayments, recentPayments, revenueByMonth,] = await Promise.all([
            this.prisma.payment.aggregate({
                _sum: { platformFee: true },
