-- ─── Restore the rich card layout on /mission ──────────────────────────
--
-- The 20260526130000_seed_mission_page migration shipped the mission
-- copy using plain <h3>+<p>+<ol> tags because StaticPageRenderer didn't
-- yet style the design's `.steps-box` and `.pillar` card layouts. The
-- renderer now carries those styles (matching commit), so this
-- migration rewrites the body to use the proper card markup — same
-- prose content, richer visual treatment.
--
-- IMPORTANT: the .pillar and .steps-box markup uses raw <div class="...">
-- elements, which Tiptap's StarterKit strips on save. If an admin opens
-- /admin/static-pages, edits the mission page, and clicks save, the
-- visual cards will be destroyed and the body will revert to plain
-- prose. For prose-only edits, prefer adjusting this row directly via
-- SQL or re-running a corrections migration. The mission page is
-- rarely-touched brand copy, so the trade-off is acceptable.
...
<div class="steps-box">
  ...
  <div class="step">
    ...
    <div class="step-body">
      <strong>Application &amp; Background Review</strong>
      <span>Every practitioner submits a detailed application including training history, certifications, years of practice, and areas of specialization. We review each application individually — no automated approvals.</span>
    </div>
  </div>
  <div class="step">
    ...
    <div class="step-body">
      ...
      <span>We contact certifying bodies, training schools, and professional associations directly to confirm the validity of credentials. Verified practitioners receive a ✦ badge on their profile.</span>
    </div>
  </div>
  <div class="step">
    <div class="step-num">3</div>
    <div class="step-body">
      <strong>Ethics &amp; Approach Interview</strong>
      <span>We conduct a personal conversation with each practitioner about their ethical framework, how they handle boundaries, and how they approach vulnerable clients. This is non-negotiable.</span>
    </div>
  </div>
  <div class="step">
    <div class="step-num">4</div>
    <div class="step-body">
      <strong>Community References</strong>
      <span>We collect and review references from previous clients and colleagues. Ongoing community feedback through our review system ensures continued accountability.</span>
    </div>
  </div>
  <div class="step">
    ...
    <div class="step-body">
      ...
      <span>Verification is not a one-time event. We monitor practitioner reviews, respond to community concerns, and conduct periodic re-verification to ensure standards are maintained.</span>
    </div>
  </div>
</div>
